...
flowchart TD
    ...
    Codec --> Transport["传输层<br/>IBacnetTransport / BacnetTransportBase<br/>BacnetIpUdpProtocolTransport / BacnetIpV6UdpProtocolTransport<br/>BacnetScTransport (WebSocket/TLS)<br/>命名空间：System.IO.BACnet"]
...
| 决策 | 选项 | 选择 | 理由 |
|------|------|------|------|
| invoke-id 管理 | 全局共享 / 按设备隔离 | 按设备隔离 | `ConcurrentDictionary<BacnetAddress, Byte>` 按远程设备独立分配，消除并发冲突 |
...
| `BacnetClient.StartCOVAutoRenewal()` | NewLife.BACnet | COV 订阅生命周期自动续约定时器 |
...
## 13. SEC 安全通信 BACnet/SC

### 13.1 职责

基于 TLS/WebSocket 的 BACnet Secure Connect 安全通信，提供端到端加密传输。

### 13.2 核心组件

| 组件 | 所在工程 | 说明 |
|------|----------|------|
| `BacnetScTransport` | NewLife.BACnet | WebSocket 传输层实现，支持 Hub/Node 和直连两种模式 |
| `BVLCSC` | NewLife.BACnet | BACnet/SC BVLC 帧编解码（BVLL 类型 0x83） |
| `BacnetBvlcScFunctions` | NewLife.BACnet | SC 功能码枚举（Hub/Peer-to-Peer 等） |
| `BacnetBvlcScResults` | NewLife.BACnet | SC 操作结果码 |

### 13.3 设计决策

| 决策 | 选项 | 选择 | 理由 |
|------|------|------|------|
| 传输协议 | 原始 WebSocket / WSS | WSS (TLS 1.2+) | 符合 BACnet/SC 标准要求，支持 wss:// 加密 |
| Hub/Node 模式 | 嵌入式 Hub / 独立 Hub | 两者都支持 | 直连模式适用于简单拓扑，Hub 模式适用于星型多节点 |
| .NET Framework 兼容 | 支持 / 不支持 | net45 不支持 | WebSocket 客户端仅 netstandard2.0+ 可用，net45 抛出 PlatformNotSupportedException |

---

## 14. GAP 标准 BACnet 服务补齐

### 14.1 职责

补齐 Who-Has/I-Have、时间同步、设备通信控制等标准 BACnet 服务。

### 14.2 核心组件

| 组件 | 所在工程 | 说明 |
|------|----------|------|
| `BacnetClient.WhoHas()` | NewLife.BACnet | 发送 Who-Has 请求（按对象 ID 或名称） |
| `BacnetClient.OnIHave` | NewLife.BACnet | 接收 I-Have 响应事件 |
| `BacnetClient.TimeSynchronize()` | NewLife.BACnet | 发送时间同步请求 |
| `BacnetClient.DeviceCommunicationControl()` | NewLife.BACnet | 发送设备通信控制请求 |
| `BacServer.OnWhoHas` | NewLife.BACnet | 服务端 Who-Has 事件处理与 I-Have 响应 |
| `BacServer.OnTimeSynchronize` | NewLife.BACnet | 服务端时间同步事件处理 |
| `BacServer.OnDeviceCommunicationControl` | NewLife.BACnet | 服务端设备通信控制事件处理 |

---

## 15. SCH Schedule/Calendar 支持

### 15.1 职责

提供 BACnet 日程表和日历数据类型的编解码支持，用于楼宇自动化定时场景。

### 15.2 核心组件

| 组件 | 所在工程 | 说明 |
|------|----------|------|
| `BacnetDailySchedule` | NewLife.BACnet | 每日日程表，由 `BacnetTimeValue` 对序列组成 |
| `BacnetSpecialEvent` | NewLife.BACnet | 特殊事件，含日历条目、时间值列表和优先级 |
| `BACnetCalendarEntry` | NewLife.BACnet | 日历条目（日期/日期范围/周NDay 三种类型） |
| `BacnetTimeValue` | NewLife.BACnet | 时间-值对数据类型 |

---

## 16. ASY Async/Await 异步 API

### 16.1 职责

在 `BacnetClient` 上提供完整的 Async/Await 异步方法，消除同步阻塞模型在高并发场景的线程开销。

### 16.2 核心组件

| 组件 | 所在工程 | 说明 |
|------|----------|------|
| `BacnetAsyncResult` | NewLife.BACnet | 基于 `TaskCompletionSource` 的异步结果封装，保留 `IAsyncResult` 兼容 |
| `BacnetClient.ReadPropertyAsync()` | NewLife.BACnet | 异步读属性（含 CancellationToken 支持） |
| `BacnetClient.WritePropertyAsync()` | NewLife.BACnet | 异步写属性 |
| `BacnetClient.SubscribeCOVAsync()` | NewLife.BACnet | 异步 COV 订阅 |
| `BacClient.ReadPropertyAsync()` | NewLife.BACnet | 高级客户端异步读属性（支持字符串地址） |
| `BacClient.WritePropertyAsync()` | NewLife.BACnet | 高级客户端异步写属性 |
...
| 异步模型 | async/await / ManualResetEvent | 两者兼有 | `BacnetAsyncResult` 保留 `IAsyncResult` 兼容；同时基于 `TaskCompletionSource` 提供完整的 Async/Await 方法（含 CancellationToken 支持） |
